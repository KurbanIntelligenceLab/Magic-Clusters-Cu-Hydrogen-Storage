...
def read_energies_file(file_path):
...
def read_formation_energies_file(file_path):
    """Read the Formation_energy-EF.txt file and return a dictionary of structure base names and their properties."""
    formation_energies = {}
    h2_energy = None
    with open(file_path, 'r') as f:
        lines = f.readlines()
    for line in lines[1:]:
        ...
        if len(parts) >= 5:
            ...
            h2_energy = float(parts[3])  # last H2 value (should be the same for all rows)
    return formation_energies, h2_energy

def gather_rotations(xyz_dir, base_name):
    rotations = []
    for i in range(6):
        xyz_path = os.path.join(xyz_dir, f"{base_name}_rot{i}.xyz")
        image_path = os.path.join(xyz_dir.replace('xyz_files', 'images'), f"{base_name}_rot{i}.png")
...
def main():
    ...
    formation_energies, h2_energy = read_formation_energies_file(os.path.join(base_dir, "Formation_energy-EF.txt"))
    structure_dirs = [
    ...
    for dir_name, has_energy in structure_dirs:
        ...
        for file in os.listdir(xyz_dir):
            if file.endswith('.xyz') and '_rot' in file:
                base_name = file.replace('_rot0.xyz', '').replace('_rot1.xyz', '').replace('_rot2.xyz', '').replace('_rot3.xyz', '').replace('_rot4.xyz', '').replace('_rot5.xyz', '')
                ...
                base_for_formation = base_name.split('-H2')[0] if '-H2' in base_name else base_name
