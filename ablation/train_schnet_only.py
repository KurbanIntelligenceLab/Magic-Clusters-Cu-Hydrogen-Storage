...
from model_schnet_only import SchNetOnlyModel
...
class SchNetDataset(Dataset):
    """
    PyTorch Dataset for SchNet-only data (geometric features and target).
    """
    ...
    def __getitem__(self, idx: int):
        ...
        return schnet_data, target, node["id"]


def collate_fn(batch: List[Any]):
    """Collate function for DataLoader to batch SchNet data."""
    schnet_data_list, target_list, node_ids = zip(*batch)
    schnet_batch = Batch.from_data_list(schnet_data_list)
    targets = torch.stack(target_list)
    return schnet_batch, targets
...
def train_one_epoch(
    ...
    model.train()
    for schnet_batch, targets in train_loader:
        schnet_batch = schnet_batch.to(config["device"])
        targets = targets.to(config["device"])

        optimizer.zero_grad()
        predictions = model(schnet_batch)
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for schnet_batch, targets in test_loader:
            schnet_batch = schnet_batch.to(config["device"])
            targets = targets.to(config["device"])
...
def train_and_eval(
    ...
    """
    Train and evaluate the SchNet-only model using leave-one-out cross-validation.
    Args:
        kg_nodes: List of knowledge graph nodes.
        kg_edges: List of knowledge graph edges (not used in SchNet-only model).
        ...
        for test_id in config["leave_out_ids"]:
            ...
            for target_key in config["target_keys"]:
                ...
                result_dir = os.path.join(
                    "results_schnet_only", f"seed_{seed}", test_id, target_key
                )
                ...
                train_dataset = SchNetDataset(
                    train_nodes,
                    target_key,
                    target_scaler=scalers[target_key],
                )
                test_dataset = SchNetDataset(
                    [test_node],
                    target_key,
                    target_scaler=scalers[target_key],
                )
                ...
                model = SchNetOnlyModel(
                    schnet_out=config["model_params"]["schnet_out"],
                    num_targets=1,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train and evaluate the SchNet-only model."
    )
    parser.add_argument(
        "--config",
        type=str,
